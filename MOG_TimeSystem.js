 ...
 * HISTÓRICO
 * =============================================================================
 * v1.6 - Bug fix by TIKA - add default values to add_hour, add_day, add_month,
 * 		  add_year so that these methods don't return NaN when called without
 * 		  parameters.
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
	...
	if (command === "show_clock") {
		$gameSystem._time_window_visible = true;
	};
	if (command === "hide_clock") {
		$gameSystem._time_window_visible = false;
	};
	if (command === "enable_time") {
		$gameSystem._time_sys_active = true;
	};
	if (command === "disable_time") {
		$gameSystem._time_sys_active = false;
	};
	return true;
};
...
Game_Map.prototype.setup = function (mapId) {
	...
	if ($gameSystem._time_data) {
		this.set_time_sys_map()
	};
};
...
Game_Map.prototype.set_time_sys_map = function (mapId) {
	...
	this.notetags().forEach(function (note) {
		...
		if (note === "<Disable Tint Screen>" || String(Moghunter.time_tint_mode) != "true") {
			...
			$gameSystem._time_data[4] = false;
			this._timesys_tintscreen = false;
		};
	}, this);
	if (this._timesys_tintscreen) {
		$gameSystem.time_system_clear()
	};
};
...
Game_Interpreter.prototype.command223 = function () {
	if ($gameMap._timesys_tintscreen) {
		return true
	};
...
Game_Switches.prototype.onChange = function () {
	...
	if ($gameSystem._time_data) {
		$gameSystem.set_base_time_phase();
	};
};
...
Game_Variables.prototype.setValue = function (variableId, value) {
	...
	if ($gameSystem._time_data && this.is_time_variable(variableId)) {
		$gameSystem.refresh_time(0, variableId);
		$gameSystem.set_base_time_phase();
	};
};
...
Game_Variables.prototype.is_time_variable = function (variableId) {
	if (Moghunter.sec_variableId === variableId) {
		return true
	};
	if (Moghunter.min_variableId === variableId) {
		return true
	};
	if (Moghunter.hour_variableId === variableId) {
		return true
	};
	if (Moghunter.day_variableId === variableId) {
		return true
	};
	if (Moghunter.month_variableId === variableId) {
		return true
	};
	if (Moghunter.year_variableId === variableId) {
		return true
	};
	if (Moghunter.day_week_variableId === variableId) {
		return true
	};
	if (Moghunter.season_variableId === variableId) {
		return true
	};
	return false;
};
...
Game_System.prototype.setup_time_system = function () {
	...
	if (String(Moghunter.time_stop_message) === "true") {
		this._stop_time_message = true
	};
	if (String(Moghunter.time_stop_interpreter) === "true") {
		this._stop_time_interpreter = true
	};
...
Game_System.prototype.setup_switch_tm = function () {
	...
	if (Moghunter.day_week_switches.length > 0) {
		this.set_time_var(this._day_week_switches, Moghunter.day_week_switches, 1);
	};
	if (Moghunter.month_switches.length > 0) {
		this.set_time_var(this._month_switches, Moghunter.month_switches, 1);
	};
	if (Moghunter.season_switches.length > 0) {
		this.set_time_var(this._season_switches, Moghunter.season_switches, 1);
	};
...
Game_System.prototype.set_time_var = function (object, value, type) {
	...
	if (type === 0) {
		for (var i = 0; i < s.length; i++) {
			object.push(String(s[i]));
		};
	} else {
		for (var i = 0; i < s.length; i++) {
			object.push(Number(s[i]));
		};
	};
};
...
// * Add Hour
//==============================
Game_System.prototype.add_hour = function (value = 0) {
	if (value <= 0) {
		return
	};
	$gameVariables._data[this._hour_variableId] += value;
	if ($gameVariables._data[this._hour_variableId] >= this.max_time(this._hour_variableId)) {
...
// * Add Day
//==============================
Game_System.prototype.add_day = function (value = 0) {
	if (value <= 0) {
		return
	};
	$gameVariables._data[this._day_variableId] += value;
	if ($gameVariables._data[this._day_variableId] >= this.max_time(this._day_variableId)) {
...
// * Add Month
//==============================
Game_System.prototype.add_month = function (value = 0) {
	if (value <= 0) {
		return
	};
	...
	if ($gameVariables._data[this._month_variableId] >= this.max_time(this._month_variableId)) {
...
// * Add Year
//==============================
Game_System.prototype.add_year = function (value = 0) {
...
Game_System.prototype.add_season = function (value) {
	...
	if ($gameVariables._data[this._season_variableId] >= this.max_time(this._season_variableId)) {
		this.refresh_time(1, this._season_variableId)
	};
...
Game_System.prototype.refresh_season = function () {
	for (var i = 0; i < this._season_switches.length; i++) {
		$gameSwitches._data[Number(this._season_switches[i])] = false
	};
	$gameSwitches._data[Number(this._season_switches[$gameVariables._data[this._season_variableId]])] = true;
	for (var i = 0; i < this._month_switches.length; i++) {
		$gameSwitches._data[Number(this._month_switches[i])] = false
	};
...
Game_System.prototype.day_week_name = function () {
	if (this._day_week_names[this.day_week() - 1]) {
		...
		return "Day Week " + String(this.day_week())
	}
};
...
Game_System.prototype.season_name = function () {
	if (this._season_names[this.season() - 1]) {
		...
		return "Season " + String(this.season())
	};
};
...
Game_System.prototype.month_name = function () {
	if (this._month_names[this.month() - 1]) {
		...
		return "Month " + String(this.month())
	};
};
...
Game_System.prototype.update_seconds = function () {
	...
	if ($gameVariables._data[this._sec_variableId] >= this.max_time(this._sec_variableId)) {
...
Game_System.prototype.check_max_time = function (parameter) {
	if ($gameVariables._data[parameter] >= this.max_time(parameter) || $gameVariables._data[parameter] < 0) {
		if (this._hour_variableId === parameter || this._month_variableId === parameter) {
			...
			if ($gameVariables._data[parameter] <= 0) {
				$gameVariables._data[parameter] = 1
			};
		} else {
...
Game_System.prototype.day_phase_effect = function () {
	...
	if (this.allow_tint_screen()) {
		$gameScreen.startTint(this.set_tint_phase(), this.day_phase_transition_speed());
	};
	$gameMap.requestRefresh();
};
...
Game_System.prototype.update_time_system = function () {
	if (!this.allow_time_system()) {
		return
	};
	this.update_seconds();
	if (this.day_phase() != this.day_phase_old()) {
		this.day_phase_effect();
	};
};
...
Game_System.prototype.allow_time_system = function () {
	...
	if (!this._time_sys_active) {
		return false
	};
	if (SceneManager.isSceneChanging()) {
		return false
	};
	if (this._stop_time_interpreter && $gameMap.isEventRunning()) {
		return false
	};
	if (this._stop_time_message && $gameMessage.isBusy()) {
		return false
	}
	return true;
};
...
Scene_Map.prototype.createDisplayObjects = function () {
	...
	if (Moghunter.timeWindow_map === "true") {
...
Scene_Map.prototype.terminate = function () {
	...
	if ($gameMap._timesys_tintscreen) {
		$gameSystem._tone_data = $gameScreen._tone;
	};
};
...
Window_Time_Status.prototype.need_fade = function () {
	if ($gamePlayer.screen_realX() < this._window_size[0]) {
		return false
	};
	if ($gamePlayer.screen_realX() > this._window_size[2]) {
		return false
	};
	if ($gamePlayer.screen_realY() < this._window_size[1]) {
		return false
	};
	if ($gamePlayer.screen_realY() > this._window_size[3]) {
		return false
	};
	if (this.opacity < 100) {
		return false
