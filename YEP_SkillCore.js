 ...
 * ============================================================================
 * 
 * <WeaponHeight: 150> will offset the position of the weapon by 150 pixels downward in the y-axis
 * <WeaponRight: 150> will offset the position of the weapon by 150 pixels in the x-axis
 * note: You might need to make sure to set <WeaponRight: 0> at the end of the sequence **
 * ============================================================================
 * Changelog
 * ============================================================================
 *
 * version 1.12.elblanquito
 * - Added ability to define <WeaponRight > and <WeaponHeight > in notetags
 * and added corresponding change to YEP_Battlecore.js to accomodate new functionality
 * 
 * Version 1.12:
...
DataManager.isDatabaseLoaded = function() {
  ...
  var  weaponHeight = 0;       //I set these as global variables, they are accessed outside of scope
  var  weaponRight = 0;        //by Yanfly.weaponHeight elBlanquito, 3DEC2017 and must be
  var  animationRight = 0;     //manually set back to 0 at the end of any sequence they are used in.
  var  animationHeight = 0;    //This is done by using <AnimationRight: 0> in the note section of Skill
  var  dummy = 0;
...
DataManager.processSkillNotetags = function(group) {
  ...
  var noteWeaponAnchorRight = /<(?:WeaponRight):[ ]([-]?\d+)>/i;   //added by elblanquito on 1DEC2017
  var noteWeaponAnchorHeight = /<(?:WeaponHeight):[ ]([-]?\d+)>/i; //  [-]?\d+ added on 3DEC2017
  var noteWeaponAnimationRight = /<(?:AnimationRight):[ ]([-]?\d+)>/i;
  var noteWeaponAnimationHeight = /<(?:AnimationHeight):[ ](\d+)>/i; //add by elblanquito on 3DEC2017
...
//added by elblanquito on 1DEC2017
	else if (line.match(noteWeaponAnchorRight)) {       //this parses for the regex stored in 
        Yanfly.weaponRight = parseInt(RegExp.$1);           //noteWeaponAnchorRight above ^
      } 
	else if (line.match(noteWeaponAnchorHeight)) {
        Yanfly.weaponHeight = parseInt(RegExp.$1);	    //these are stored in global variables
      }							    //Yanfly.animationHeight is how you access
	else if (line.match(noteWeaponAnimationHeight)) {   //them in other scripts. Added on 3DEC2017
        Yanfly.animationHeight = parseInt(RegExp.$1);
      }
	else if (line.match(noteWeaponAnimationRight)) {    //note: remember to manually set them
        Yanfly.animationRight = parseInt(RegExp.$1);	    //back to <AnimationRight: 0> at the end of using them
      }							    //or there might be artifacts from it!
//
