...
*   ---
*
*   ShowCommonEventImage
*   - This will allow you to decide if you want to show image for 
*   common event menu option. NOTE: Image MUST have same name as the Common Event.
*   And make sure this plugin command is before AddCommonEventMenu command in CE.
*   Accepts one param: true | false
*   eg ShowCommonEventImage true
*
*   ---
*
*   CommonEventMenuImageSrc
*   - This will allow you to add image source for ShowCommonEventMenuImage.
*     This only works in combination with: ShowCommonEventMenuImage
*     Accepts one param path to folder with images. Keep in mind that 'img/' will be prpended to your path. 
*     eg  CommonEventMenuImageSrc sv_actors - This will pull images from img/sv_actors
*     eg  CommonEventMenuImageSrc sv_actors/menuImages - This will pull images from img/sv_actors/menuImages
*
*   ---
*
* ============================================================================
...
* ============================================================================
*
* Version 1.04(By Tika):
* - Added option to add images to each option of the common event menu.
*
* Version 1.03:
...
Yanfly.Param.CEMShowImages = false;
Yanfly.Param.CEMImagesSource = '';
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  } else if (command === 'ShowCommonEventImage') {
    this.setImageVariable(this.argsToString(args));
  } else if (command === 'CommonEventMenuImageSrc') {
    this.setImagesSource(this.argsToString(args));
  } else if (command === 'SetMenuItemIndex') {
    $gameTemp.__yepCEMIndex = parseInt(args[0]);
  }
};

Game_Interpreter.prototype.setImagesSource = function (value) {
  Yanfly.Param.CEMImagesSource = value;
};

Game_Interpreter.prototype.setImageVariable = function (value) {
  Yanfly.Param.CEMShowImages = value == 'true';
};
...
Window_CommonEventMenu.prototype.setup = function (mapId, eventId) {
  ...
  this.open();
  if($gameTemp.__yepCEMIndex > 0) {
    this.select($gameTemp.__yepCEMIndex - 1);
    $gameTemp.__yepCEMIndex = 0;
  }
  else
    this.select(0);
  this.refresh();
...
Window_CommonEventMenu.prototype.drawItem = function (index) {
  ...
  if (Yanfly.Param.CEMShowImages)
    this.drawImage(index, rect.x, rect.y, rect.width, rect.height);
};

var Window_CommonEventMenu_itemRect = Window_CommonEventMenu.prototype.itemRect;
Window_CommonEventMenu.prototype.itemRect = function (index) {
  if (Yanfly.Param.CEMShowImages) {
    var rect = new Rectangle();
    var maxCols = this.maxCols();
    rect.width = this.itemWidth();
    rect.height = this.height / 2 - this._padding;
    rect.x = index % maxCols * (rect.width + this.spacing()) - this._scrollX;
    rect.y = Math.floor(index / maxCols) * rect.height - this._scrollY;
    return rect;
  } else {
    return Window_CommonEventMenu_itemRect.call(this, index);
  }
};

var Window_CommonEventMenu_itemRectForText = Window_CommonEventMenu.prototype.itemRectForText;
Window_CommonEventMenu.prototype.itemRectForText = function (index) {
  if (Yanfly.Param.CEMShowImages) {
    ...
    return Window_CommonEventMenu_itemRectForText.call(this, index);
  }
};

Window_CommonEventMenu.prototype.drawImage = function (index, _x, _y, rectWidth, rectHeight) {
  var name = $dataCommonEvents[this._list[index].ext].name;
  var bitmap = ImageManager.loadPicture('../' + Yanfly.Param.CEMImagesSource + '/' + name);
  if (!bitmap) return;
  var that = this;
  bitmap.addLoadListener(function () {
    var width = 120;
    var height = 120;
    var x = _x + rectWidth / 2 - width / 2;
    var y = _y + that.padding + that.lineHeight();
